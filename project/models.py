...
class Genre(models.Base):  # Модель для жанров
    __tablename__ = 'genres'

    name = Column(String(100), unique=True, nullable=False)


class Director(models.Base):  # Модель для директора
    __tablename__ = 'directors'

    name = Column(String(100), unique=True, nullable=False)


class Movie(models.Base):  # Модель для фильмов
...
class User(models.Base):  # Модель для юзеров
    __tablename__ = 'user'
