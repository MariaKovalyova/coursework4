...
def __generate_password_digest(password: str) -> bytes:  # Генерация пароля
...
def generate_password_hash(password: str) -> str:  # Генерация хэша пароля
    return base64.b64encode(__generate_password_digest(password)).decode('utf-8')


def compare_passwords_hash(password_hash, other_password) -> bool:  # Сравнения пароля и пароля их хэша
    return password_hash == generate_password_hash(other_password)


def generate_tokens(email, password, password_hash=None, is_refresh=False):  # Генерация Токенов

    if email is None:  # Проверка данных
        return None
    ...
    minIS = datetime.datetime.utcnow() + datetime.timedelta(minutes=current_app.config['TOKEN_EXPIRE_MINUTES'])
    data['exp'] = calendar.timegm((minIS.timetuple()))
